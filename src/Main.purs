...
import Plot.JobBatcher (JobQueue, addManyPlots, addPlot, cancelAll, cancelWithBatchId, hasJobs, initialJobQueue, runFirst)
    ...
    HandleQueue -> do
      when (hasJobs state.queue) do
        ...
        case maybeJobResult of
          ...
          Just jobResult -> do
            if jobResult.job.batchId == clearPlotBatchId 
              then
                H.modify_ (_ { queue = newQueue, clearPlot = jobResult.drawCommands } )
              else
                H.modify_ (_ { queue = newQueue, plots = alterPlot (updateExpressionPlotCommands jobResult.drawCommands) jobResult.job.batchId state.plots })
