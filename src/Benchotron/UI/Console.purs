...
import Benchotron.Core (BenchmarkResult, Benchmark, BenchM, runBenchM, runBenchmark, unpackBenchmark)
import Benchotron.StdIO (stdoutWrite, stderrWrite, question)
import Benchotron.UI.Constants (outputFolder)
...
runSuite bs = do
  ...
  go b = do
    ...
    when (not exists) (mkdir outputFolder)
    benchmarkToFile b (outputFolder <> "/" <> slug b <> ".json")
